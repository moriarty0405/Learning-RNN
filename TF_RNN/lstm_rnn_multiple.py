...
from tensorflow.contrib import rnn ## rnn move to tensorflow.contrib
...
# placeholder contains the whole state
...
# Unpack columns (No need for them)
...
# Tensorflow suggested way to create multiple layer of RNN with dropout
def lstm_cell():
...
# reshape of states_series
logits = tf.matmul(states_series, W2) + b2 #Broadcasted addition
labels = tf.reshape(batchY_placeholder, [-1])

# Calculating loss
logits_series = tf.unstack(tf.reshape(logits, [batch_size, truncated_backprop_length, num_classes]), axis=1) #Broadcasted addition
